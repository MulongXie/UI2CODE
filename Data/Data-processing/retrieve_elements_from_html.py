...
url = 'D:/git_file/github/doing/UI2CODE-IMGCAPTION/Data-processing/19.html'
...
if __name__ == '__main__':
    ...
    # set output
    path_org = pjoin(root, 'org.png')
    path_html = pjoin(root, 'html.html')
    path_label = pjoin(root, 'label.csv')
    path_drawn = pjoin(root, 'drawn.png')
    ...
    try:
        ...
        element_all = fetch_element('a', element_all)
        element_all = fetch_element('h1', element_all)
    ...
    driver.quit()
    # driver = webdriver.Chrome(driver_path, options=options)
    print("Time taken:%.3fs" % ((time.clock() - start_time)))
    print(time.ctime())
